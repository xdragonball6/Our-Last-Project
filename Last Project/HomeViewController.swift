...
extension HomeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "sgDetail" {
            // MainBtnCollectionView 셀 클릭 처리
            if let cell = sender as? UICollectionViewCell, let indexPath = self.MainBtnCollectionView.indexPath(for: cell) {
            ...
            } else if let cell = sender as? UICollectionViewCell, let indexPath = MainReservationCollectionView.indexPath(for: cell), let destinationVC = segue.destination as? MainPageListListTableTableViewController, indexPath.row < mainPageDataDown.count {
                // MainReservationCollectionView 셀 클릭 처리
                // 데이터가 존재하고 유효한 경우 'userID' 값을 전달
...
//           let indexPath = MainReservationCollectionView.indexPath(for: cell),
//           let destinationVC = segue.destination as? MainPageListListTableTableViewController,
//           indexPath.row < mainPageDataDown.count {
//            // 데이터가 존재하고 유효한 경우에만 전달
//            let selectedData = mainPageDataDown[indexPath.row]
//            destinationVC.dataToDisplay = selectedData
//        }
//    }
//}
